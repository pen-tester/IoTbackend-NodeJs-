...
$(document).ready(function() {
  ...
  $('#add_venue').click(function(){
    ...
    if (selected_venue) {
      ...
      var html = '<div class="device" id="' + venuename + '" data-bnew="1" data-id="' + sensorid + '" data-desc="' + desc + '" data-size="' + size + '" data-no="' + venue_newno + '"> \
  ...
  $('#campus_submit').click(function(evt) {
    ...
    var campus = {
      ...
      old_campus_id: $('#oldcampus_id').val(),
    ...
    for (i=0; i < devices.length; i++ ){
      tmp = {
        ...
        size: devices[i].style.width,
        old_venue_id: old_venues[$(devices[i]).attr('data-no')]
      }
      if ($(devices[i]).attr('data-bnew')) {
        tmp.notify_option = {
          brightness: 0,
          vacant_col : '#81EF90',
          occupied_col : '#ff0000',
  ...
  venue_newno = $('#oldnew_no').val();
  var devices = $('.device');

  for (i=0; i < devices.length; i++ ){
    old_venues[$(devices[i]).attr('data-no')] = $(devices[i]).attr('data-id');
  }
});